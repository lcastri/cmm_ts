...
class sIAED(MyModel):
    ...
    def get_model(self, config) -> Model:
        inp = Input(shape = (self.config[W_SETTINGS][W_NPAST], self.config[W_SETTINGS][W_NFEATURES]))
        x = IAED(config, self.target_var)(inp)
        m = Model(inp, x)
        return m


    def search_best_param(self, X_train, y_train, X_val, y_val):
        param_grid = {
                      'LSTMunit': [128, 64, 32],
                      'D1unit': [256, 128, 64, 32],
                      'D2unit': [256, 128, 64, 32],
                      }

        hypermodel = lambda x: self.get_model(config = x)
